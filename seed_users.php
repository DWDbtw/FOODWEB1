...
function ensureRoleColumn($con) {
    $sql = "SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'users' AND COLUMN_NAME = 'role'";
...
try {
    ...
    foreach ($accounts as $acc) {
        ...
        if ($found) {
            ...
            echo "User exists (id={$user_id}, username={$acc['username']}) — updating password/role/email...\n";
    ...
    $con->commit();
    echo "Done. Three accounts ensured. Default passwords: Client123!, Manager123!, Admin123!\n";
