...
<p style="color: #1c2329; font-weight: 450;">
                    		Только свежая рыба и морепродукты — никаких компромиссов. Каждый ролл собирается вручную из отборных ингредиентов прямо перед подачей.
                    	</p>
	                    </div>
	                </div>
				</div>
...
<p style="color: #1c2329; font-weight: 450;">
                    		Привезём ваш заказ горячим и смвежим за 40 минут. Собственные курьеры, термосумки и удобное отслеживание на карте в реальном времени.
                    	</p>
	                    </div>
	                </div>
				</div>
...
<p style="color: #1c2329; font-weight: 450;">
                    		Авторские рецепты нашего шеф-повара сочетают японские традиции с современными акцентами — каждый укус запоминается надолго.
                    	</p>
	                    </div>
	                </div>
				</div>
...
if($.trim(contact_name) == "")
	...
	<script type="text/javascript">
	    $(document).ready(function()
	    {
			...
			{
				// If the click originated from an edit control, ignore to prevent adding to cart
				if ($(e.target).closest('.quick-edit, .edit-pencil').length) {
					return;
				}
				...
				$.ajax({
					url: 'add_to_cart.php',
					type: 'POST',
					data: { action: 'add', menu_id: menu_id },
					success: function(response)
					{
						if (response.success)
						{
							var toast = $('#cart-toast');
							toast.text(menu_name + ' добавлено в корзину');
							toast.fadeIn(200);
